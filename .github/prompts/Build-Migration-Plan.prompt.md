...
description: "Consumes the Discovery Dossier + Capability Matrix and produces a finalized, evidence-backed migration plan with phase sequencing, squad assignments, target Azure architecture, and quality gates."
---
...
3. **Sequence the execution plan** across Phases 1–6, dispatching the right squad specialists per the Capability Matrix
...
Run **after** `/assess-any-application` has produced `reports/Discovery-Dossier.md` and `reports/Capability-Matrix.yaml`. Trigger commands: `/build-migration-plan`, `@squad build migration plan`.
...
- **Refine** if you see evidence the discovery missed — log the refinement in `.squad/decisions.md` as `Architect-refined-<app>: from=<X> to=<Y> reason=<Z>` and update the dossier inline
...
## Step 7 — Update `.squad/decisions.md`
