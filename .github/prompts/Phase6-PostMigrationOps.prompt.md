...
- Recommend `@squad run cost optimization review` for deeper FinOps work if costs are a concern
- Recommend `@squad run security hardening review` for a focused security review if needed
- Recommend `@squad show migration status` to review the full migration status dashboard
...
- [ ] Next steps clearly communicated (`@squad run cost optimization review`, `@squad run security hardening review`, `@squad show migration status`)
