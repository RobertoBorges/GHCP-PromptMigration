...
If the application hasn't completed Phase 1, recommend: `@squad I need to assess my legacy app first` before proceeding.
...
**Severity checkpoint:** If you discover exposed secrets, credentials in source code, or public admin endpoints during scope review, immediately flag as 🔴 Critical and alert the user before continuing. Recommend `@squad evaluate rollback options` if the finding affects a production system.
...
**Squad dispatch:** If Defender recommendations require Azure resource configuration changes, note these for the Azure Specialist agent. Do not modify Azure resources directly — document the required changes and recommend: `@squad apply Defender recommendations for [resource]` after user approval.
...
**Squad dispatch:** For Key Vault configuration or managed identity setup, note for the Azure Specialist agent.
...
**Squad dispatch:** For NSG/firewall rule changes and private endpoint setup, note for the DevOps Engineer agent.
...
**Squad dispatch:** For dependency updates and vulnerable package remediation, note for the Coder agent. For container image scanning, note for the DevOps Engineer agent.
...
|---|---|---|
| Code vulnerabilities, dependency updates | Coder (Rusty Ryan) | `@squad fix vulnerable dependencies in [app]` |
| Azure resource misconfiguration | Azure Specialist (Basher Tarr) | `@squad apply security config for [resource]` |
| Network/firewall changes | DevOps Engineer (Turk Malloy) | `@squad harden network security for [app]` |
| Rollback consideration | Cutover Commander (Reuben Tishkoff) | `@squad evaluate rollback options for [app]` |
| Monitoring gaps | Observability (Livingston Dell) | `@squad set up security monitoring for [app]` |
...
- For ongoing hardening: `@squad run post-migration operations review`
- If critical findings require rollback: `@squad evaluate rollback options`
- To check overall status: `@squad show migration status`
- If stakeholder reporting needed: `@squad generate security posture summary slide`
...
- [ ] Next steps clearly communicated (with `@squad` commands for each)
