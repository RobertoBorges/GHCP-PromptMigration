...
description: |
  Generate an executive-ready Migration Strategy Report (HTML deck) from a customer's portfolio artifacts — CMDB exports, RVTools/Azure Migrate inventories, DMA outputs, meeting notes, vendor proposals, architecture diagrams.
  **Use when:** A consultant, solution architect, or CSA needs to produce a CIO-ready migration plan for a customer portfolio (apps, databases, infrastructure, or any mix). Starts the Portfolio Planning flow for multi-app engagements — produces an executive deck and a structured handoff file that feeds into the per-app modernization flow.
  **Triggers on:** "generate migration report", "analyze CMDB", "create portfolio analysis", "migration strategy report for [CUSTOMER]", "analyze application portfolio", "create migration deck", "analyze what's in this folder", "build a deck from these artifacts", "analyze infrastructure for migration", "database migration strategy", "RVTools export analysis", "Azure Migrate data analysis", "portfolio assessment".
  **Covers:** Auto-detection of workload pillars (Apps / DB / Infra), CAF-aligned 6 Rs classification, deterministic Factory / ISD-Partner / Unknown execution ownership, phased roadmap, dependency mapping, move groups, risks, executive HTML output styled for leadership presentation.
