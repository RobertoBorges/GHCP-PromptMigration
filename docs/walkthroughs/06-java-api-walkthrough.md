...
```text
@agent migrate Use-cases/06-JavaAPI to Azure Container Apps plus Azure PostgreSQL — full pipeline. Assess the app, upgrade it to Spring Boot 3 on Java 21, plan the JPA and PostgreSQL move, generate the Azure deployment path, deploy it, wire CI/CD, and finish with monitoring guidance. Fan out.
```
**Expected artifacts**
...
**Follow-up prompts**
- `@agent show me the minimum viable CI/CD flow first, then show me the hardened version.`
...
- `@agent we are stuck on PostgreSQL compatibility. Replan the next three moves and fan out only where it saves time.`
- `@agent keep the REST contract stable, minimize downtime, and call out any decision that changes cost or risk.`
...
## 💡 Power-User Shortcut
> Old advanced equivalents, if you already know them: QuickAssessment, Phase1-Plan, DatabaseMigration, Phase2-MigrateCode, Phase3-GenerateInfra, Phase4-DeployToAzure, Phase5-SetupCICD, Phase6-PostMigrationOps.
