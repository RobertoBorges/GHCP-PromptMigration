...
Canonical format:
```
- <ISO-8601-UTC> | actor=QuickAssessment | action=<verb-phrase> | files=<+created,~modified,-deleted> | tokens=~<bucket> | turn=<n> | notes="<free text>"
```

Rules:
- Use `actor=QuickAssessment` for actions taken by this prompt.
