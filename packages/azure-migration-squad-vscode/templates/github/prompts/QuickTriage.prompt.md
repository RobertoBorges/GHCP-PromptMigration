...
Use this prompt when the user needs a fast go/no-go signal, an intake summary for a backlog, or a quick cross-technology screen before investing in deeper assessment work. Run it with `@agent run quick triage`.
...
Example: `Go with Conditions - WebForms and Forms Auth are the main blockers; run @agent assess WebForms migration next.`
...
- Recommend the next prompt from this set when applicable: `@agent assess .NET upgrade`, `@agent assess Java upgrade`, `@agent assess WCF migration`, `@agent assess WebForms migration`, `@agent assess Classic ASP migration`, `@agent run Phase 1 plan and assess`
