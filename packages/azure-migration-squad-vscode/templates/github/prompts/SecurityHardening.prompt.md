...
Canonical format:
```
- <ISO-8601-UTC> | actor=SecurityHardening | action=<verb-phrase> | files=<+created,~modified,-deleted> | tokens=~<bucket> | turn=<n> | notes="<free text>"
```

Rules:
- Use `actor=SecurityHardening` for actions taken by this prompt.
