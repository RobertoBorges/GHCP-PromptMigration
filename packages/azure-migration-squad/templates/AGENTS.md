...
6. **Read `.github/copilot-instructions.md`** — Contains the natural-language ↔ slash-command mapping table. Use it to interpret user intent regardless of which surface they're on (CLI vs Chat).

## CLI vs Chat — Important

This squad lives on two surfaces:

- **GitHub Copilot CLI** (the `copilot` terminal command). Users CANNOT type slash commands like `/assess-any-application` here — Copilot CLI doesn't auto-register `.github/prompts/*.prompt.md` files. They will say what they want in natural language ("assess this application", "discover this app", "phase 2"). Map their intent to the right prompt using the table in `.github/copilot-instructions.md`.
- **VS Code Copilot Chat** — slash commands DO work here directly.

**Both should produce identical outcomes.** If a user on CLI types `/assess-any-application` literally, treat it as the natural-language equivalent.
