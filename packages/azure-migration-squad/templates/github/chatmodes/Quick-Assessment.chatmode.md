...
- priority risks and likely next squad handoff
...
5. recommended next squad member or `@squad` command
...
- Hand to `@squad run database migration review` when data movement or schema strategy is the dominant risk.
- Hand to `@squad run security hardening review` when identity, secrets, or compliance are the main blocker.
- Hand to `@squad run Phase 1 plan and assess` when the quick triage shows the team needs the full assessment workflow.
