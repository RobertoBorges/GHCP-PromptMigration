...
Trigger commands: `/assess-any-application`, `@squad assess any application`, `@squad discover this app`.
...
Next: @squad architect approve discovery
Then: /build-migration-plan
```

Also append a one-line entry to `.squad/decisions.md`:
...
- If the user explicitly waives discovery, log it in `.squad/decisions.md` with reduced-confidence acknowledgement and proceed with the matrix marked `evidence_confidence: low` across the board.
...
- [ ] `.squad/decisions.md` updated with one-line entry
- [ ] Handoff note delivered (`@squad architect approve discovery` → `/build-migration-plan`)
