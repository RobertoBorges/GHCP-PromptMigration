...
- Recommend `@squad run Phase 1 plan and assess` when deeper analysis is needed.

## Completion Guidance
At the end:
- State the complexity score and effort band plainly
- Call out the top 3 blockers or accelerators
- Recommend `@squad run Phase 1 plan and assess` for the full structured workflow
- Recommend `@squad run database migration review` if the database is a primary complexity driver
- Recommend `@squad show migration status` if status tracking is already in use
...
- [ ] Next steps clearly communicated (`@squad run Phase 1 plan and assess`, `@squad run database migration review`, `@squad show migration status`)
