...
> The canonical list of major architecture decisions the Azure Migration Squad asks the user about. Used by Phase 1 to generate `reports/Decisions-Required.md`, and referenced by every Phase prompt for hard-gate enforcement.
...
Drives SKU tier selection. Without this, the squad cannot validate that the proposed architecture fits budget.
...
Every recommendation in this catalog is a **default guess**, not a "best choice." The squad surfaces evidence from the Capability Matrix but explicitly defers to the user. See [`decision-hardstop.md`](./decision-hardstop.md) for the binding behavior.
